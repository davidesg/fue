 ...
 *   Modified for FUE: Windows/MSYS2 support, time series plot functions.  *
 *                                                                         *
...
)
{
    ...
    if (title == NULL) {
     ...
     f(x) notitle ls 2, \
     bu(x) notitle ls 1,\
     bl(x) notitle ls 1 ", cmd, n+1, -cmax, cmax,  name, title, handle->pstyle) ;
    }
...
)
{
    ...
    if (title == NULL) {
        sprintf(line, "%s [-%.1f:%.1f][-%.1f:%.1f] \"%s\" with %s 1 31" , cmd, AbsMax, AbsMax, AbsMax, AbsMax, name, handle->pstyle) ;
    } else {
        sprintf(line, "%s [-%.1f:%.1f][-%.1f:%.1f] \"%s\" title \"%s\" with %s 1 31", cmd, AbsMax, AbsMax, AbsMax, AbsMax, name,
                      title, handle->pstyle) ;
    }
