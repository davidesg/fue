...
/*new*/	 else if ((f==1) && (n>200)) gnuplot_cmd(h,"set size 1.52,.975");
	 ...
	 if(f>4)
       ...
       if(f>4)
       gnuplot_cmd(h,"set title '%s' 40.65,3.75 font 'bold,24'", ser->name);
       else
       gnuplot_cmd(h,"set title '%s' 26,0.95 font 'bold,24'", ser->name);
	 ...
	 {
	 gnuplot_cmd(h,"set xlabel '@^{/E=24  -}_{}{/Times-Roman=21 w} ( @^^{/Symbol=18 \331}_{}{/Symbol=22 s}_{@^{/E=22  -}_{}{/Times-Roman=18 w}} ) = %2.2lf{/Arial %} (%2.2f{/Arial %})&{def}&{def}&{def}&{def}&{def}@^^{/Symbol=18 \331}_{}{/Symbol=22 s}_{/Times-Roman=18 w} =  %2.2f{/Arial %} ' -2,-3 font 'Arial,19'",
                     100*ser->mean/refactor,  100*(Stdev( ser->data, ser->nobs) / sqrt( ser->nobs ))/refactor, 100*(Stdev( ser->data, ser->nobs))/refactor); 
	 }
...
/*new*/	else if  (( f==1 ) && (n > 200))
  ...
  if((f>4)|| (f==1) && (n>200))
  gnuplot_cmd(h,"set title 'acf' -2.2,-.21 font 'Arial,20'");
  else
  gnuplot_cmd(h,"set title 'acf' -2.05,-.21 font 'Arial,18'");
